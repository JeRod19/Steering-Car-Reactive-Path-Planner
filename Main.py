...
def check_future_collision(grid, robot, path, index):
    robot_pts =  np.array([[-robot.width*0.6, 0], [0, 0], [robot.width*0.6, 0]])
    ...
    for i in range(30):
        ...
        dx = end[1] - init[1]
        dy = end[0] - init[0]
        seg_length = np.linalg.norm([dy, dx])
...
senseMap = env.enviroment('maps/RobotMap.jpg', scale)
...
while not robot.check_goal(goal):
    ...
    if abs(orient_diff) < 179 * m.pi / 180:
